...
BEGIN
    ...
    *,
    ...
    -- extract a part of prd_key, in order to join data together
    ...
    SELECT sls_prd_key FROM bronze.crm_sales_details) -- products don't have orders
    */
    ...
    sls_prd_key,
    sls_cust_id,
    CASE WHEN sls_order_dt = 0 OR LEN(sls_order_dt) != 8 THEN NULL -- CAST can|t directly change the integer into date
         ELSE CAST(CAST(sls_order_dt AS VARCHAR) AS DATE) 
    END AS sls_order_dt,

    CASE WHEN sls_ship_dt = 0 OR LEN(sls_ship_dt) != 8 THEN NULL -- CAST can|t directly change the integer into date
         ELSE CAST(CAST(sls_ship_dt AS VARCHAR) AS DATE) 
    END AS sls_ship_dt,

    CASE WHEN sls_due_dt = 0 OR LEN(sls_due_dt) != 8 THEN NULL -- CAST can|t directly change the integer into date
